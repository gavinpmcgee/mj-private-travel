...
(function () {
  ...
  var CROSS =
    ...
    'stroke-linecap="square" aria-hidden="true"><path d="M5 5l14 14M19 5L5 19"/></svg>';
  ...
  function render() {
    ...
    /* Host pages can override display:none, so the add control is detached
       outright rather than just hidden. Nothing can reveal what isn't there. */
    var canAdd = state.tripType === "multi" && state.legs.length < CONFIG.maxLegs;
    addBtn.hidden = !canAdd;
    if (canAdd && !addBtn.parentNode) {
      legsEl.parentNode.insertBefore(addBtn, legsEl.nextSibling);
    } else if (!canAdd && addBtn.parentNode) {
  ...
  function buildLeg(leg, i) {
    ...
    el.innerHTML =
      '<div class="cq-leg-head">' +
        ...
        (canDrop ? '<button type="button" class="cq-leg-drop" data-drop ' +
          'aria-label="Remove leg ' + String(i + 1).padStart(2, "0") + '" ' +
          'title="Remove this leg">' + CROSS + '</button>' : '') +
      '</div>' +
